...
const Spark = () => {
    ...
    return (
        <div className='flex flex-col gap-5 lg:px-[7%] px-[4%] mt-16'>
            <p className='lg:text-[42px] text-[32px] text-white'>From <span className='text-[#60a6e7]'>Spark</span> to <span className='text-[#60a6e7]'>Spotlight:</span> we take  you <br className='lg:block hidden' /> every step of the way to success.</p>
                ...
                <div className='w-full'>
                    <div className='border-[2px] border-[#B3D0F2] p-2 w-full overflow-hidden rounded-full h-[65px] flex'>
                        <div className='flex flex-row w-full justify-between items-center'>
                            {
                                tabs.map((t, index) => (
                                    <div key={index} onClick={() => handleClick(t.act)} className={`w-full  cursor-pointer flex items-center justify-center rounded-full py-3 font-bold ${active === t.act ? "bg-[#B3D0F2] text-black" : "text-white"}`}>{t.name}</div>
                                ))
                            }
                        </div>
                    </div>
